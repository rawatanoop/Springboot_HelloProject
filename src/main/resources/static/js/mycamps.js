...
$(document).ready(function(){
  ...
  $.get("donationCamp/user" ,function(data, status){
    $.each(data, function (i, item) {
      ...
      queue += '<h4 class="panel-title"> From - '+ start.toDateString()+ '  to - '+ end.toDateString() ;
      queue += '</h4>';
      queue += '</div>';
      ...
      $.get("donationCamp/volunteer" ,{campID:data[i].id  ,requestStatus:"Request"},function(data, status){
        $.each(data, function (i, item) { 
          ...
          $('#reject_'+i+'_'+data[i].campID).click(function(){
            var userID = data[i].id;
            var campID =data[i].campID;
            var unitDonate=data[i].unitDonate;
            $.ajax({
      ...
      $.get("donationCamp/volunteer" ,{campID:data[i].id  ,requestStatus:"Accept"},function(data, status){
        $.each(data, function (i, item) { 

          var row = '<tr>';
          row +='<td>'+(i+1)+'</td> <td> '+data[i].name+' </td> ';
          row +='<td>'+(i+1)+'</td> <td> accepted </td> ';
          row +='</tr>';
          $('#detailsTable_'+data[i].campID).append(row); 
